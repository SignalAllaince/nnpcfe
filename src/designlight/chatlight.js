...
const Chatlight = ({ handleOut, userName, userEmail }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.error('Error fetching bot reply:', error);
      const errorMessage = !navigator.onLine ? 'Please check your internet connection' : 'An error occurred';
      const botReplyError = { type: 'bot', text: errorMessage };
      setMessages(prevMessages => [...prevMessages.slice(0, -1), botReplyError]);
    }
    inputRef.current.value = '';
  ...
  const processBotReply = (response) => {
    ...
    formattedResponse = formattedResponse.replace(/(\d+)\. /g, '<div class="numbered-item"><strong>$1.</strong> ');

    // Convert bullet points
    formattedResponse = formattedResponse.replace(/- /g, '<div class="bullet-item">&#8226; ');

    // Close any opened div tags
    formattedResponse = formattedResponse.replace(/<\/div>$/, '');

    // Convert URLs to clickable links
    const urlRegex = /(https?:\/\/[^\s]+)/g;
    formattedResponse = formattedResponse.replace(urlRegex, '<a href="$1" class="link-button" target="_blank" rel="noopener noreferrer">Click Here</a>');
  ...
  return (
    <section>
      <div className='nav-header-contents'>
        <div>
          <img className='navs-nnpc-logo' src={logo7} alt="NNPC Logo" />
        </div>
        <div>
          <p className='nav-p'>INEO</p>
        </div>
